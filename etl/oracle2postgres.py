...
def create_connection(database='gestao'):
    if (database == 'gestao'):
...
def import_orgaos(appconnection, appcursor):
    ...
    for each in oracle_cursor:
        ...
        if level == 1:
            appcursor.execute("INSERT INTO orgaos_orgao (nome, sigla, nivel ) VALUES ( %s , %s , %s ) ",
                              (nome, sigla, level,))
        else:
            appcursor.execute("INSERT INTO orgaos_orgao (nome, sigla, nivel , parent_id )"
                              "SELECT  %s , %s , %s , o.id from orgaos_orgao o "
                              "WHERE o.sigla = %s ", (nome, sigla, level, opai))
    appconnection.commit()


def import_funcionarios(appconnection, appcursor):
    ...
    for each in oracle_cursor:
        ...
        print(each)
        appcursor.execute(" INSERT INTO pessoas_pessoa (matricula , login , nome, foto, salario, estatutario)"
                          " values (%s ,%s , %s , %s , %s, 0 )",
                          (matricula, login, nome, fotostream, salario,)
                          )
        appcursor.execute(
            " update pessoas_pessoa set orgao_id = (select id from orgaos_orgao where nome = %s ) where matricula = %s ",
            (orgao, matricula,)
        )
    appconnection.commit()


def atualiza_salarios(appconnection, appcursor):
    oracle_cursor.execute(funcionarios_sql)
    for each in oracle_cursor:
        login = each[1]
        salario = each[6]
        appcursor.execute(
            " update pessoas_pessoa set salario = %s where login = %s ", (salario, login,)
        )
    appconnection.commit()
...
def import_responsavel(appconnection, appcursor):
    ...
    for each in oracle_cursor:
        ...
        appcursor.execute(
            ...
            " responsavel_id = (select id from pessoas_pessoa where matricula = %s ) "
            " where nome = %s ",
...
if __name__ == "__main__":
    _appconnection, _appcursor = create_connection()
    # import_orgaos(_appconnection, _appcursor)
    # import_funcionarios(_appconnection, _appcursor)
    # import_responsavel(_appconnection, _appcursor)
    # atualiza_salarios(_appconnection, _appcursor)
